...
@section('title', 'Editar Trimestre - Sistema de Gestión Escolar')
            ...
            <form action="{{ route('trimestres.update', $trimestre) }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="nombre" name="nombre" 
                               value="{{ old('nombre', $trimestre->nombre) }}" required>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select name="id_anio" id="id_anio" class="form-select" required>
                            ...
                            @foreach($anios as $anio)
                                <option value="{{ $anio->id_anio }}" {{ old('id_anio', $trimestre->id_anio) == $anio->id_anio ? 'selected' : '' }}>
                                    {{ $anio->anio }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control" id="fecha_inicio" name="fecha_inicio" 
                               value="{{ old('fecha_inicio', $trimestre->fecha_inicio) }}" required>
                    </div>
                    <div class="col-md-6">
                        <label for="fecha_fin" class="form-label">Fecha de Fin <span class="text-danger">*</span></label>
                        <input type="date" class="form-control" id="fecha_fin" name="fecha_fin" 
                               value="{{ old('fecha_fin', $trimestre->fecha_fin) }}" required>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end mt-4">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save me-1"></i> Actualizar Trimestre
                    </button>
                </div>
            </form>
        </div>
